...
let handleUserPublished = async (user, mediaType) => {
    ...
    // Subskrybuj zarówno wideo, jak i audio
    ...
    // Jeśli dostępny jest strumień wideo, to go odtwarzaj
    if (mediaType === 'video') {
        user.videoTrack.play(`user-${user.uid}`)
    }

    // Jeśli dostępny jest strumień audio, to go odtwarzaj
    if (mediaType === 'audio') {
...
let handleUserLeft = async (user) => {
    // Usuwamy użytkownika z obiektów i DOM
    ...
    // Jeśli użytkownik opuścił, musimy przywrócić strumień, gdy ponownie dołączy
    await client.publish([localTracks[0], localTracks[1]]);
}
